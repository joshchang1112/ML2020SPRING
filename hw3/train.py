...
optimizer = torch.optim.Adam(model.parameters(), lr=5e-5) # optimizer 使用 Adam
#optimizer = torch.optim.Adam(model.parameters(), lr=5e-4) # optimizer 使用 Adam
#scheduler = torch.optim.lr_scheduler.StepLR(optimizer, 20 * 300, gamma=0.6)
...
for epoch in range(num_epoch):
    ...
    if epoch == 120:
        ...
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, 350 * 50, eta_min=1e-6)
    ...
    with torch.no_grad():
        ...
        if val_acc > best_acc:
            torch.save(model.state_dict(), 'vgg19.model')
